...
namespace UserFrosting\Sprinkle\OAuth2ServerJY\ServicesProvider;
...
use UserFrosting\Sprinkle\OAuth2ServerJY\OAuth2\UserEntity;
use UserFrosting\Sprinkle\OAuth2ServerJY\OAuth2\AccessTokenRepository;
use UserFrosting\Sprinkle\OAuth2ServerJY\OAuth2\ClientRepository;
use UserFrosting\Sprinkle\OAuth2ServerJY\OAuth2\ScopeRepository;
use Psr\Http\Message\ResponseInterface;
use Psr\Http\Message\ServerRequestInterface;
use UserFrosting\Sprinkle\OAuth2ServerJY\OAuth2\RefreshTokenRepository;
use UserFrosting\Sprinkle\OAuth2ServerJY\OAuth2\UserRepository;
use League\OAuth2\Server\Grant\AuthCodeGrant;
use League\OAuth2\Server\Grant\RefreshTokenGrant;
use League\OAuth2\Server\Middleware\AuthorizationServerMiddleware;
use UserFrosting\Sprinkle\OAuth2ServerJY\OAuth2\AuthCodeRepository;
use League\OAuth2\Server\ResourceServer;
use UserFrosting\Sprinkle\OAuth2ServerJY\Database\Models\OauthClients;
use UserFrosting\Sprinkle\OAuth2ServerJY\Database\Models\Scopes;
...
class ServicesProvider
{
    public function register($container)
    {
		$container['OAuth2'] = function ($c) {
            ...
            if($c->config['oauth2serverJY.private_key_path'] === ""){
			     $privateKeyPath = 'file://' . __DIR__ . '/../OAuth2/private.key';
            } else {
                $privateKeyPath = $c->config['oauth2serverJY.private_key_path'];
            }

            if($c->config['oauth2serverJY.public_key_path'] === ""){
			     $publicKeyPath = 'file://' . __DIR__ . '/../OAuth2/public.key';
            } else {
                 $publicKeyPath = $c->config['oauth2serverJY.public_key_path'];
            }
			...
			$OAuth2 = new AuthorizationServer(
				...
				$scopeRepository,
				$privateKeyPath,
                $c->config['oauth2serverJY.EncryptionKey']
			);
			...
			$grant = new \League\OAuth2\Server\Grant\AuthCodeGrant(
				...
				new \DateInterval($c->config['oauth2serverJY.auth_code_time'])
			);

			$grant->setRefreshTokenTTL(new \DateInterval($c->config['oauth2serverJY.refresh_token_time']));

			// Enable the authentication code grant on the server
			$OAuth2->enableGrantType(
				$grant,
				new \DateInterval($c->config['oauth2serverJY.access_token_time'])
			);
            ...
            $refreshGrant->setRefreshTokenTTL(new \DateInterval($c->config['oauth2serverJY.refresh_token_time'])); // new refresh tokens will expire after 1 month
        ...
        $container->extend('classMapper', function ($classMapper, $c) {
            $classMapper->setClassMapping('oauth_clients', 'UserFrosting\Sprinkle\OAuth2ServerJY\Database\Models\OauthClients');
            return $classMapper;
        });
    }
}
